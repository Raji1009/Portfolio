              ...
              return (
                <button
                  ...
                  className={`inline-flex items-center gap-2 rounded-full border px-4 py-2 text-sm font-semibold transition ${
                    isActive
                      ? 'border-violet-300/60 bg-violet-500/20 text-white shadow-[0_0_24px_rgba(124,58,237,0.28)]'
          ...
          <div className="space-y-6">
            {highlightSections
              ...
              .map((section) => {
                ...
                return (
                  <div key={section.id} className="rounded-3xl border border-white/10 bg-white/[0.025] p-4 md:p-5">
                    {isAllView && (
                      <div className="mb-4 flex items-center gap-3">
                        <span className={`h-2.5 w-2.5 rounded-full ${section.dotClass} ${section.glowClass}`} />
                        <p className={`text-xs font-bold uppercase tracking-[0.26em] ${section.textClass}`}>{section.label}</p>
                    ...
                    {!isAllView && (
                      <div className="mb-5">
                        <p className={`text-xs font-bold uppercase tracking-[0.26em] ${section.textClass}`}>{section.label}</p>
                        <h3 className="mt-2 text-2xl font-bold text-white">{section.summary}</h3>
                      </div>
                    )}
                    <div className={`grid gap-3 ${sectionItems.length > 1 ? 'md:grid-cols-2' : ''}`}>
                      {sectionItems.map((item, index) => (
                        <motion.article
                          ...
                          className={`glass-card relative h-full rounded-2xl border-l-4 ${section.borderClass} p-4 transition hover:-translate-y-0.5 hover:bg-white/[0.06] ${section.glowClass}`}
                        >
